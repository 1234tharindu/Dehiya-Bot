  ...
  async run(message, args) {
    ...
    try {
      ...
      if (!args[2]) {
        ...
        try {
          await channel.bulkDelete(amount).then(async (m) => {
            ...
            let embed = new EmbedBuilder()
              .setColor('0x#00ffff')
              .setDescription(`✅  Cleared **${m.size}**/**${amount}** messages in ${channel}!`);
        ...
        switch (args[2]) {
          case "--bots":
            msg = await channel.messages.fetch({ limit: amount })
            data = []
            ...
            try {
              await channel.bulkDelete(data.length ? data : 1, true).then(async (m) => {
